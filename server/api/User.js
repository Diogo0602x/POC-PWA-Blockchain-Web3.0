...
db.on("error", console.log.bind(console.error, "Falha de conexão"))
db.once("open", () => {
  ...
  if (name == "" || email == "" || password == "" || dateOfBirth == "") { 
    ...
    User.find({email}).then(result => {
      if (result.length) {
        ...
        res.json({
          status: "FAILED",
          message: "Usuário com o email já existe!"
        })  
      } else {
        ...
        bcrypt.hash(password, saltRounds).then(hashedPassword => {
          ...
          .catch(err => {
            res.json({
              status: "FAILED",
              message: "Ocorreu um erro enquanto salvava o usuário!"
            })
          })
        })
        .catch(err => {
      ...
      res.json({
        status: "FAILED",
        message: "Ocorreu um erro enquanto checava se o usuário existe"
      })
    })
   }
})
